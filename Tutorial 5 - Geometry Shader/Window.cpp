...
static TCHAR szTitle[] = _T("DirectX Tutorial #5 - Geometry Shader");
...
{
	_effect = new LineThicknessEffect(_d3dDevice, _d3dDeviceContext);
	_effect->Initialize();
}
...
LRESULT Window::WindowMessage(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		if (wParam == VK_UP)
			_lineThickness += 1.0f;
		else if (wParam == VK_DOWN && _lineThickness > 1.0f)
			_lineThickness -= 1.0f;
		Render();
		break;
